...
(0..12).each do |i|
  x = i % 4
  movie = FFMPEG::Movie.new(File.open(Rails.root.join('lib', 'seeds', 'additional_videos', "#{mp4_file[x]}")))
...
(0..12).each do |i|
  movie = FFMPEG::Movie.new(File.open(Rails.root.join('lib', 'seeds', 'additional_videos', 'final_video.mp4')))
  post = Post.create!({ 
